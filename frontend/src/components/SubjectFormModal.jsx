...
const SubjectFormModal = ({ showModal, closeModal, subject, onSuccess }) => {
  const [formData, setFormData] = useState({
    ...
    semesters: [],
    periodsPerYear: "",
    gradingType: "numerical", // Giá trị mặc định
  });
  ...
  useEffect(() => {
    ...
    if (subject) {
      setFormData({
        ...
        semesters: subject.semesters?.map((s) => s._id),
        periodsPerYear: subject.periodsPerYear || "",
        gradingType: subject.gradingType || "numerical",
      });
    } else {
      setFormData({
        name: "",
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
      <div className="relative w-full max-w-lg rounded-lg bg-white p-6 shadow-lg">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block mb-1 text-sm">Loại điểm</label>
            <select
              name="gradingType"
              ...
              className="w-full border rounded px-3 py-2"
            >
              <option value="numerical">Điểm số (0-10)</option>
              <option value="letter">Chữ (Đ/F)</option>
            </select>
          </div>
